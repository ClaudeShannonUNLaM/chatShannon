...
public class RF11Tests {
	...
	public void unidadesDeTiempo() throws ParseException, IOException {
		Assert.assertEquals(
				...
				jenkins.escuchar("@jenkins cuántas horas son 60 minutos",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas 2 horas equivalen a 7200 segundos",
				jenkins.escuchar("@jenkins cuántos segundos son 2 horas",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas 1 minuto equivalen a 60 segundos",
				jenkins.escuchar("@jenkins cuántos segundos hay en 1 minuto",USUARIO)
			);		
	}	
	...
	public void unidadesDeCapacidad() throws ParseException, IOException {
		Assert.assertEquals(
				...
				jenkins.escuchar("@jenkins cuántos mililitros son 2 litros",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas 3 litros equivalen a 300 centilitros",
				jenkins.escuchar("@jenkins cuántos centilitros son 3 litros",USUARIO)
			);	
	...
	public void unidadesDeLongitud() throws ParseException, IOException {
		...
		Assert.assertEquals(
				...
				jenkins.escuchar("@jenkins cuántos kilometros son 500 metros",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas 2 metros equivalen a 200 centimetros",
				jenkins.escuchar("@jenkins cuántos centimetros son 2 metros",USUARIO)
			);		
	}
	...
	public void MedidasIncorrectas() throws ParseException, IOException {
		Assert.assertEquals(
				...
				jenkins.escuchar("@jenkins cuántos gramos son 1 metro",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas las unidades especificadas no se pueden transformar",
				jenkins.escuchar("@jenkins cuántos kilos son 1000 litros",USUARIO)
			);
		
		Assert.assertEquals(
				"@delucas las unidades especificadas no se pueden transformar",
				jenkins.escuchar("@jenkins cuántas onzas son 1000 horas",USUARIO)
			);		
	}	
}