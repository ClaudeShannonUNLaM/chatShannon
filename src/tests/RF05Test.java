...
public class RF05Test {
	...
	public void TriviasFacts() throws ParseException, IOException {
		Assert.assertEquals(
				...
				jenkins.escuchar("@jenkins quiero datos de trivia",USUARIO).getMensaje()
			);
		
		Assert.assertEquals(
				"Rusia tiene una superficie más grande que Plutón.",
				jenkins.escuchar("@jenkins dame datos importantes",USUARIO).getMensaje()
			);
		
		Assert.assertEquals(
				"Arabia Saudí importa camellos de Australia.",
				jenkins.escuchar("@jenkins quiero saber algo interesante",USUARIO).getMensaje()
			);
		Assert.assertEquals(
